...
#data = pd.read_csv("raw_data/08_00-10_00-non-duplicated.csv")
#data = data.drop(columns=data.columns[[0,1,6,7]])
...
data = pd.read_csv("raw_data/december_23_30.csv", header=0, usecols=[0, 1, 2, 3])
...
#    return osrm_out.groupby("route_id").filter(delete_useless_data)
    osrm_out["is_deleted"] = False
    osrm_out["aux_route_ends"] = 0
    osrm_out["speed"] = 0.0
    
    osrm_out.rename(columns={"time":"time_unix"}, inplace=True)
    osrm_out["time"] = np.datetime64('NaT')
    
    osrm_out = osrm_out.groupby("route_id").apply(delete_useless_data)
    
    osrm_out = osrm_out[~osrm_out["is_deleted"]]
...
#    route_id = route_data["route_id"].iloc[0]
    ...
    route_data = route_data.drop_duplicates(subset=["time_unix"])
    route_data = route_data.drop_duplicates(subset=["distance_from_start"])
...
#            raise Exception(f"Number of gps points is less than 2: arac_id: {arac_id} route_id: {route_data['route_id'].iloc[0]} approx. time {route_data['time'].iloc[0]}")
        route_data["is_deleted"] = True
        route_data.iloc[0, route_data.columns.get_loc("aux_route_ends")] = -1
        return route_data
...
#            raise Exception(f"Route contains multiple directions arac_id: {arac_id} route_id: {route_data['route_id'].iloc[0]} approx. time {route_data['time'].iloc[0]}")
        route_data["is_deleted"] = True
        route_data.iloc[0,route_data.columns.get_loc("aux_route_ends")] = -2
        return route_data
    
    route_data = route_data.sort_values("time_unix")
...
#            raise Exception(f"start node OR end node cannot be found in list arac_id: {arac_id} route_id: {route_data['route_id'].iloc[0]} approx. time {route_data['time'].iloc[0]}")
        route_data["is_deleted"] = True
        route_data.iloc[0, route_data.columns.get_loc("aux_route_ends")] = -3
        return route_data
...
#            raise Exception(f"start node > end node arac_id: {arac_id} route_id: {route_data['route_id'].iloc[0]} approx. time {route_data['time'].iloc[0]}")
        route_data["is_deleted"] = True
        route_data.iloc[0,route_data.columns.get_loc("aux_route_ends")] = -4
        return route_data
        
    route_data.iloc[0,route_data.columns.get_loc("aux_route_ends")] = first_ii
    route_data.iloc[1,route_data.columns.get_loc("aux_route_ends")] = last_ii
    route_data["time"] = pd.to_datetime(route_data["time_unix"], format="%Y%m%d%H%M%S")
    route_data["speed"] = 1e-3 * np.diff(route_data["distance_from_start"], append=np.nan) / (np.diff(route_data["time"], append=np.datetime64('NaT')) / np.timedelta64(1, 'h'))
    route_data["acc"] = (np.diff(route_data["speed"], append=np.nan) * 1000 / 3600) / (np.diff(route_data["time"], append=np.datetime64('NaT')) / np.timedelta64(1, 's'))
    return route_data

def smooth_osrm_data(osrm_out):
    osrm_out["is_smooth"] = False
    
    osrm_out.loc[osrm_out["speed"] > 200, "is_smooth"] = True
    osrm_out.loc[osrm_out["speed"] < 0, "is_smooth"] = True
    osrm_out.loc[osrm_out["acc"].abs() > 3.5, "is_smooth"] = True
    
    n_smooth = osrm_out["is_smooth"].sum()
    
    def calculate_speed(df):
        df["speed"] = 1e-3 * np.diff(df["distance_from_start"], append=np.nan) / (np.diff(df["time"], append=np.datetime64('NaT')) / np.timedelta64(1, 'h'))
        return df
    
    def calculate_acc(df):
        df["acc"] = (np.diff(df["speed"], append=np.nan) * 1000 / 3600) / (np.diff(df["time"], append=np.datetime64('NaT')) / np.timedelta64(1, 's'))
        return df

    while n_smooth > 0:
        
        # ındex re_index
        # osrm_out = osrm_out.groupby("routttte_id").any("is_filtered").......compouted
        # SADECE İŞARETLİ ROTALARDA HESAP YAPSIN
        # DATA SİLDİKTEN SONRA 2'NIN ALTINA DÜŞERSE ROTAYI İPTAL ETSİN
        
        osrm_out = osrm_out[~osrm_out["is_smooth"]]
        
        osrm_out = osrm_out.groupby("route_id").apply(calculate_speed)
        osrm_out = osrm_out.groupby("route_id").apply(calculate_acc)
#        for route_id, route_data in osrm_out.groupby("route_id"):
#            route_data["speed"] = 1e-3 * np.diff(route_data["distance_from_start"], append=np.nan) / (np.diff(route_data["time"], append=np.datetime64('NaT')) / np.timedelta64(1, 'h'))
#            route_data["acc"] = (np.diff(route_data["speed"], append=np.nan) * 1000 / 3600) / (np.diff(route_data["time"], append=np.datetime64('NaT')) / np.timedelta64(1, 's'))
...
def augmentation(osrm_out, segments):
    ...
    for route_id, route_data in osrm_out.groupby("route_id"):
        ...
        first_ii = route_data.iloc[0, route_data.columns.get_loc("aux_route_ends")]
        last_ii = route_data.iloc[1, route_data.columns.get_loc("aux_route_ends")]
...
def get_processed_data(arac_id, arac_data, segments):
    try:
        ...
        aug_data = augmentation(osrm_data, segments)
